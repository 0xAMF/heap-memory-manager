...
void free(void* mem) { my_free(mem); }
 ...
 * @return: the address of the block wih suitable size on success, the passed
 * block address on failure */
static block_t* split(block_t** block, size_t size)
{
    ...
    block_t* block_end = (block_t*)((char*)(*block) + (*block)->meta.size + sizeof(block_t));
    ...
    block_t* newblock = (block_t*)((char*)block_end - req_size);
...
void* my_malloc(size_t size)
{
    ...
    if (size < 0) {
        pthread_mutex_unlock(
            &memory_mutex); // Unlock the mutex before returning
        return NULL;
    }
    ...
    if (baseptr == NULL) {
        ...
        while (tempbase != NULL) {
            ...
            /* if the block found is bigger than or equal to the required size and the block is free */
            if (tempbase->meta.size >= size && inuse == 0) {
                // if the block is bigger than required, then split
                ...
                } else { // if the size is exactly what we want, return the memory.
                    block = tempbase;
                }
                memory = (block + 1);
                block->meta.inuse = 1;
...
void display_list()
{
    ...
    while (tempNode != NULL) {
        printf("size = %ld\t  %16p <-  %16p   -> %16p\t inuse = %i\n",
            tempNode->meta.size, (tempNode->meta.prev), tempNode,
            (tempNode->meta.next), tempNode->meta.inuse);
